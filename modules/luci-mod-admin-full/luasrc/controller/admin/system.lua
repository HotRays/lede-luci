	...
	elseif step == 2 then
		...
		if nixio.fs.access("/rom/lib/preinit/79_disk_ready") then
			if not http.formvalue("keep") == "1" then
				erase_sda = 'mount -o remount,rw /rom && rm -f /rom/etc/sda.ready && mount -o remount,ro /rom;'
			end
		end
		fork_exec("sleep 1; killall dropbear uhttpd; %s sleep 1; /sbin/sysupgrade %s %q" %{ erase_sda, keep, image_tmp })
	end
end
